...
<!-- Sidebar drawer -->
<aside
	class={'fixed top-0 left-0 z-50 h-full w-64 transform bg-stone-200 p-4 transition-transform duration-300 ease-in-out lg:block lg:w-1/5 lg:translate-x-0 ' +
		(isOpen ? 'translate-x-0' : '-translate-x-full')}
>
	<div class="flex h-full flex-col">
		<div class="flex items-center justify-between">
			<h1 class="item-center flex gap-1">
				<p class="rounded-full bg-blue-600 p-2">
					<CubeSolid class="h-10 w-10 shrink-0" />
				</p>
		...
		<div class="flex flex-1 flex-col gap-y-5 py-8">
			{#each links as link}
				<a
					class={'flex w-full items-center gap-2 rounded-full px-3 py-2 text-base hover:bg-[#2564ed2d] lg:w-3/4 ' +
