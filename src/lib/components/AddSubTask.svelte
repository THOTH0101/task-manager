<script lang="ts">
	import { enhance } from '$app/forms';
	import { Button, Input, Label, Modal } from 'flowbite-svelte';
	import { toast } from 'svelte-sonner';
	import Loader from './Loader.svelte';
	import type { SubmitFunction } from '@sveltejs/kit';

	let { open = $bindable(), task } = $props();

	let userInput = $state({ title: '', date: '', tag: '' });
	let errors = $state({ title: '' });
	let isLoading = $state(false);

	const handleSubmit: SubmitFunction = async () => {
		isLoading = true;

		return async ({ result, update }) => {
			switch (result.type) {
				case 'success':
					open = false;
					toast.success('Subtask added successfully.');
					await update({ reset: true });
					break;
				case 'failure':
					userInput = result.data?.input;
					errors = result.data?.errors;
					toast.error(result.data?.message ?? 'Invalid credentials.');
					await update();
					break;
				default:
					break;
			}
			isLoading = false;
		};
	};
</script>

<Modal bind:open dismissable={false} class="bg-stone-200">
	<form method="POST" action={`/task/${task?.id}?/addSubTask`} use:enhance={handleSubmit}>
		<h2 class="mb-4 text-base leading-6 font-bold text-stone-900">ADD SUB-TASK</h2>
		<div class="mt-2 flex flex-col gap-6">
			<div>
				<Label class="mb-2">Title</Label>
				<Input
					placeholder="Sub-Task title"
					type="text"
					name="title"
					class="w-full rounded"
					bind:value={userInput.title}
				/>
				{#if errors}
					<span class="mt-0.5 text-xs text-[#f64949fe]">{errors.title}</span>
				{/if}
			</div>

			<div class="flex items-center gap-4">
				<Label class="mb-2">Task Date</Label>
				<Input
					placeholder="Date"
					type="date"
					name="date"
					class="w-full rounded"
					bind:value={userInput.date}
				/>
				<Label class="mb-2">Tag</Label>
				<Input
					placeholder="Tag"
					type="text"
					name="tag"
					class="w-full rounded"
					bind:value={userInput.tag}
				/>
			</div>
		</div>

		<div class="mt-4 flex gap-4 py-3 sm:flex-row-reverse">
			{#if isLoading}
				<Loader />
			{:else}
				<Button
					color="blue"
					type="submit"
					class="cursor-pointer bg-blue-600 px-8 text-sm font-semibold text-stone-50 hover:bg-blue-700 sm:w-auto"
				>
					Submit
				</Button>

				<Button
					color="light"
					type="button"
					class="cursor-pointer bg-stone-100 px-5 text-sm font-semibold text-stone-900 sm:w-auto"
					onclick={() => {
						userInput = { title: '', date: '', tag: '' };
						errors = { title: '' };
						open = false;
					}}
				>
					Cancel
				</Button>
			{/if}
		</div>
	</form>
</Modal>
